...
export async function create(data) {
    ...
    const categoryDoc = await categoryRepo.findByIdOrNameAndType(data.category, "investment");
...
export async function getByCategoryAndMonth(categoryId, year, month) {
    ...
    const categoryDoc = await categoryRepo.findByIdOrNameAndType(categoryId, "investments");
...
export async function modifyInvestment(id, { updateData, mode }) {
    ...
    if (updateData.category) {
        const categoryDoc = await categoryRepo.findByIdOrNameAndType(updateData.category, "investment");
        if (!categoryDoc) {
            throw new Error("INVALID_CATEGORY");
        }
