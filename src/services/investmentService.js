...
export async function modifyInvestment(id, { updateData, mode }) {
    ...
    if (isTargetOnce && !isNewOnce) {
        ...
        await investmentRepo.deleteById(target._id);
        ...
        return await investmentRepo.createMany(investmentsToCreate);
    }

    if (frequencyChanged && target.seriesId) {
        switch (mode) {
            case 'ALL': {
                const currentSeries = await investmentRepo.findBySeries(target.seriesId);
                ...
                await investmentRepo.deleteAllInSeries(target.seriesId);
                ...
                return await investmentRepo.createMany(items);
            }

            case 'FUTURE': {
                await investmentRepo.deleteFutureInSeries(target.seriesId, target.dueDate);
            ...
            case 'PAST': {
                await investmentRepo.deletePastInSeries(target.seriesId, target.dueDate);
            ...
            case 'SINGLE':
            default: {
                return await investmentRepo.update(id, {
                    ...updateData,
    ...
    switch (mode) {
        ...
        case 'FUTURE':
            await investmentRepo.updateFutureInSeries(target.seriesId, target.dueDate, updateData);
            return await investmentRepo.findBySeries(target.seriesId, target.dueDate);

        case 'PAST':
            await investmentRepo.updatePastInSeries(target.seriesId, target.dueDate, updateData);
            return await investmentRepo.findBySeries(target.seriesId);
