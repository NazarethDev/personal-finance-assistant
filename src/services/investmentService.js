...
export async function create(userId, data) {
    ...
    const categoryDoc = await categoryRepo.findByIdOrNameAndType(data.category, "investment", userId);
    ...
    if (!isRecurrent) {
        const createdInvestment = await repo.createSingle({
            ...data,
            user: userId,
            category: categoryId,
            seriesId: null
        ...
        return await createdInvestment.populate("category");
    }
    ...
    await repo.createMany(investmentsToCreate);
...
export async function getByCategoryAndMonth(userId, categoryId, year, month) {
    ...
    const categoryDoc = await categoryRepo.findByIdOrNameAndType(categoryId, "investment", userId);
...
export async function removeInvestment(userId, id, mode) {
...
export async function modifyInvestment(userId, id, payload) {
    ...
    if (updateData.category) {
        const categoryDoc = await categoryRepo.findByIdOrNameAndType(updateData.category, "investment", userId);
        if (!categoryDoc) {
            throw new Error("INVALID_CATEGORY");
        }
